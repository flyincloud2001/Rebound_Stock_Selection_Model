...
# 第二部分是每日分析 按下按鈕後 對這100支ETF抓最新資料 找出當日z_score和自己專屬觸發門檻最接近的前十名
...
def load_candidates():
    ...
    這個檔案裡每一列是一支ETF 包含它專屬的trigger_zscore(觸發門檻) optimal_days(最佳持有天數)等參數
    """
...
def fetch_today_status(row):
    ...
    再算出跟這支ETF專屬trigger_zscore的絕對距離
    ...
    # 這樣才能跟排名時的定義維持一致 不會用到不同的統計基準
    z_today = (today_return - row["return_mean"]) / row["return_std"]

    # 這支ETF專屬的觸發門檻是負的trigger_zscore 例如trigger_zscore是2.5 代表門檻是-2.5
    # distance是今天的z_score跟這個門檻的絕對距離 越小代表越接近
    # 距離小不代表今天一定會反彈 只代表今天的狀況比較接近這支ETF歷史上表現最好的那種情境
    distance = abs(z_today - (-row["trigger_zscore"]))
...
def plot_top_etfs(top_df):
    """
    畫出前十名ETF的今日z_score和它們專屬觸發門檻(負的trigger_zscore)的對照長條圖
    可以直接看出每支ETF今天實際落點跟門檻差多少
    """
    fig, ax = plt.subplots(figsize=(10, 5))
    x = np.arange(len(top_df))

    ax.bar(x - 0.2, top_df["z_today"], width=0.4, label="今天的z_score")
    ax.bar(x + 0.2, -top_df["trigger_zscore"], width=0.4, label="專屬觸發門檻")

    ax.set_xticks(x)
    ax.set_xticklabels(top_df["symbol"], rotation=45)
    ax.set_ylabel("z_score")
    ax.legend()
    ax.set_title("前十名ETF 今日z_score與專屬觸發門檻對照")
...
def main():
    ...
    with tab1:
        ...
        st.caption("排名依據best_rebound_ratio 來自過去10年歷史資料 不會隨最新股價變動")
    ...
    with tab2:
        st.subheader("執行每日分析")
        st.write("按下按鈕後 會對所有候選ETF抓取最新資料 找出當日z_score和自己專屬觸發門檻最接近的前十名")
        st.caption("提醒 距離最近不代表這支ETF在其他觸發門檻下不會有更好的表現 只代表今天的狀況最接近它歷史上表現最好的情境")
        ...
        if st.button("開始分析"):
            ...
            st.subheader("今日z_score與專屬觸發門檻對照圖")
